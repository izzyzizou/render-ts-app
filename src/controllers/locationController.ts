...
export const updateLocation = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // Check geofences
    const geofencesResult = await db.query(
      'SELECT * FROM geofences WHERE family_id IN (SELECT family_id FROM family_members WHERE user_id = $1)',
      [userId]
    );
    ...
    for (const geofence of geofencesResult.rows) {
      ...
      if (distance <= geofence.radius_meters) {
        // In a real app, we would check if they just entered
        console.log(`User ${userId} is inside geofence ${geofence.name}`);
